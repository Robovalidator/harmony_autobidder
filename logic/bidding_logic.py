...
VALIDATOR_LENGTHS: List[int] = []
MAX_VALIDATOR_LENGTHS: int = 20


def get_validators_and_bid_if_necessary(bidding_enabled: bool = False) -> Dict[str, Any]:
    ...
    if validator_lower_bid:
        ...
        if (
            ...
            response_json["action"] = f"Lowering the bid by adding key {key_to_add}"
    ...
    if validator_increase_bid:
        ...
        if (
            ...
            if not prevent_bid_due_to_inefficient:
                response_json["action"] = f"Increasing the bid by removing key {key_to_remove}"
...
def should_show_response_json(prev_response_json: Optional[Dict[str, Any]], 
                            response_json: Dict[str, Any]) -> bool:
    if not response_json:
        raise RuntimeError(f"response_json should be populated: {response_json}")
